(ns eutros.starmetallic.lib.subclass
  (:import (clojure.java.api Clojure)
           (clojure.lang IFn Compiler$HostExpr RT)
  ...
  (let [opts (reduce (fn [col [k v]]
    ...
    (doseq [{name :name getter :get setter :set} exposed-fields]
      (let [reflected ^Field (get-target-field super name)
        ...
        (when (and setter
                   (not (Modifier/isFinal (.getModifiers reflected))))
          ;; generate setter
